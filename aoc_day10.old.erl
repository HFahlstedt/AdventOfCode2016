...
solve_part1() -> initial_state().


start() -> 
    register(bot_controller, spawn(aoc_day10, bot_controller, [[], give_instructions()])),
    init_bots(value_instructions()).

init_bots([]) -> [];
init_bots([{value, Value, Bot}|IRest]) -> bot_controller ! {set_value, Bot, Value},
                                          init_bots(IRest).

bot_controller(Bots, Instructions) -> 
    receive 
        {set_value, Bot, Value} -> case lists:keyfind(Bot, 1, Bots) of
                                        false -> Pid = spawn(aoc_day10, bot, [Bot, []]),
                                                 bot_controller(Bots ++ [{Bot, Pid}]);
                                        {Bot, Pid} -> Pid ! {assign, Value},
                                                      bot_controller(Bots)
                                   end;
        {bot_ready, Bot, Values} -> case list:keyfind(Bot, 2, Instructions) of
                                        false -> io:format("Should not happend~n");
                                        {give, Bot, {T1, High}, {T2, Low}} -> give_value()

bot(Id, Values) ->
    receive
        {assign, Value} -> case Values of
                                [Val] -> bot_controller ! {bot_ready, Id, [Val, Value]},
                                         bot(Id, []);
                                [] -> bot(Id, [Value])
                           end
    end.
...
is_value_setter(Instruction) -> case Instruction of
                                    {value, _, _} -> true;
                                    _ -> false
                                end.

initial_state() -> initial_state(value_instructions(), dict:new()).

initial_state([], Dict) -> Dict; 
initial_state([{value, Val, Bot}|Rest], Dict) -> case dict:is_key(Bot, Dict) of 
                                                    true -> initial_state(Rest, dict:store(Bot, dict:fetch(Bot, Dict) ++ [Val], Dict));
                                                    false -> initial_state(Rest, dict:store(Bot, [Val], Dict))
                                                 end.

give_instructions() -> lists:filter(fun(I) -> case I of {give, _, _, _} -> true; _ -> false end end, instructions()).

value_instructions() -> lists:filter(fun(I) -> is_value_setter(I) end, instructions()).
