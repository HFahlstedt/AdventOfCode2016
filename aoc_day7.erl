...
solve_part1() -> length(filter(fun (X) -> supports_tls(X) end, ips())).

solve_part2() -> length(filter(fun (Y) -> hasABAandBAB(element(1, Y), element(2, Y)) end, lists:map(fun(X) -> {getABAorBAB(X, true), getABAorBAB(X, false)} end, ips()))).

supports_tls(String) -> supports_tls(String, false, false).

supports_tls([], _, HasAbba) -> HasAbba;
supports_tls([A, A, A, A|Tail], InHyperSeq, HasAbba) -> supports_tls(Tail, InHyperSeq, HasAbba);
supports_tls([A, B, B, A|_], true, _) -> false;
supports_tls([A, B, B, A|Tail], false, _) ->  supports_tls(Tail, true, true);
supports_tls([X|Tail], InHyperSeq, HasAbba) ->
    case [X] of
        "[" -> supports_tls(Tail, true, HasAbba);
        "]" -> supports_tls(Tail, false, HasAbba);
        _ -> supports_tls(Tail, InHyperSeq, HasAbba)
    end.

getABAorBAB(String, GetABA) -> getABAorBAB(String, false, GetABA).

getABAorBAB([], _, _) -> [];
getABAorBAB([A, A, A|Tail], InHyperSeq, GetABA) -> getABAorBAB([A|Tail], InHyperSeq, GetABA);
getABAorBAB([A, B, A|Tail], InHyperSeq, GetABA) when InHyperSeq /= GetABA -> [{A, B, A} | getABAorBAB([B, A |Tail], InHyperSeq, GetABA)];
getABAorBAB([A, B, A|Tail], InHyperSeq, GetABA) -> getABAorBAB([B, A|Tail], InHyperSeq, GetABA);
getABAorBAB([X|Tail], InHyperSeq, GetABA) ->
    case [X] of
        "[" -> getABAorBAB(Tail, true, GetABA);
        "]" -> getABAorBAB(Tail, false, GetABA);
        _   -> getABAorBAB(Tail, InHyperSeq, GetABA)
    end.

hasABAandBAB([], _) -> false;
hasABAandBAB([{A, B, _}|ABAsTail], BABs) -> 
    case lists:any(fun ({X, Y, _}) -> A == Y andalso B == X end, BABs) of
        true -> true;
        false -> hasABAandBAB(ABAsTail, BABs)
    end.
