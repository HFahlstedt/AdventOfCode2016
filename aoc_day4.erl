...
solve_part1() -> lists:foldl(
    fun ([_, Id, _], Acc) -> Acc + Id end, 
    0, 
    real_rooms()).

solve_part2() -> lists:filter(fun({Names, _}) -> lists:any(fun(Y) -> Y == "northpole" end, Names) end, lists:map(
    fun([NameList, Id, _]) -> { (lists:map(fun(X) -> decrypt(X, Id) end, NameList)), Id } end, 
        real_rooms())).

real_rooms() -> lists:filter(fun ([NameList, _, Checksum]) -> is_room_real(NameList, Checksum) end, rooms()).

decrypt(CipherText, Id) -> lists:map(fun(X) -> $a + ((X - $a + Id) rem 26) end, CipherText).

is_room_real(NameList, Checksum) -> string:sub_string(get_checksum(NameList), 1, length(Checksum)) == Checksum.

get_checksum(List) -> get_checksum(List, []).

get_checksum([], Acc) -> lists:map(fun({Key, _}) -> Key end, lists:sort(fun (A, B) -> compare_tuple(A, B) end, count_chars(Acc)));
get_checksum([L|LTail], Acc) -> get_checksum(LTail, lists:sort(L ++ Acc)). 

count_chars(String) -> count_chars(String, lists:nth(1, String), 0, []).

count_chars([], Last, Count, Acc) -> [{Last, Count}] ++ Acc;
count_chars([S|Stail], Last, Count, Acc) when S == Last -> count_chars(Stail, Last, Count + 1, Acc);
count_chars([S|Stail], Last, Count, Acc) -> count_chars(Stail, S, 1, [{Last, Count}] ++ Acc).

compare_tuple({Akey, Acount}, {Bkey, Bcount}) -> 
